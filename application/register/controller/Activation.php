...
class Activation extends controller
{
    ...
    public function index(){
        ...
        $mail->Subject = "小印相 -- 请尽快验证你的帐号"; // 邮件标题
        $mail->Body = "注册成功，你在本站注册的邮箱需要验证！请点击 http://inxiang.net/register/checkNum/?num=$number (或者复制到浏览器打开)，完成验证！"; // 邮件正文

        if ($mail->send()) {
            echo "<script>alert('邮件已经发送，请尽快查阅');window.open('','_self').close()</script>";
        } else {
